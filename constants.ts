export const SHADERS = {
  ...
  display: `
    ...
    uniform sampler2D u_overlay_depth;
    uniform sampler2D u_bg_texture;
    uniform sampler2D u_bg_depth;
    uniform vec2 u_uvScale;
    uniform vec2 u_mouse;
    uniform float u_threshold;
    uniform float u_softness;

    vec2 deform(sampler2D depthTex, vec2 uv, vec2 mouse) {
        float d = texture2D(depthTex, uv).r;
        float intensity = 0.05; // Adjust intensity as needed
        // Fix for opposite direction: Invert the displacement vector
        return uv - (mouse - 0.5) * d * intensity;
    }

    void main () {
        // Correct aspect ratio for "cover" effect
        vec2 uv = (vUv - 0.5) * u_uvScale + 0.5;
        
        // Displacement
        vec2 overlayUv = deform(u_overlay_depth, uv, u_mouse);
        vec2 bgUv = deform(u_bg_depth, uv, u_mouse);

        vec3 C = texture2D(u_output_texture, vUv).rgb;
        float density = max(C.r, max(C.g, C.b));
        
        vec3 overlayColor = texture2D(u_overlay_texture, overlayUv).rgb;
        vec3 bgColor = texture2D(u_bg_texture, bgUv).rgb;
        
        // Smoother blending using smoothstep
        float alpha = 1.0 - smoothstep(u_threshold, u_threshold + u_softness, density);
        
        // Mix background and overlay
        vec3 finalColor = mix(bgColor, overlayColor, alpha);
